---
name: cozeni-setup
description: Cozeni導入プロンプトに従い、既存のJavaScriptサイトへ購入者認可をフレームワークに合わせて実装する。Next.js App Routerとその他のJavaScript / TypeScriptサーバー構成を判定し、対応する実装資料を案内する。導入後に「購入できない」と相談された場合の切り分けも扱う。
---
