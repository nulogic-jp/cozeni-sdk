...
- **データ取得・Server Action（redirectしない）**: 各入口で独立に認可する。`nextEntitlement()`（redirectしない）や、上記の保護データ取得ヘルパーを再利用し、拒否は`AccessDenied`で受け取って画面・戻り値へ変換する。Server Actionの副作用には、購入者認可に加えて既存のCSRF / Origin検査を維持する。
- **Route Handler（JSON API、redirectしない）**: リダイレクトせず、`denialResponse(entitlement)`で401/403/503のJSONへ`enter_url`を含めて返す。fetchの呼び出し元をHTMLへ飛ばさないため、Route Handlerの中では`redirect()`を呼ばない。
